...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubProfile = onSnapshot(profileRef, (doc) => {
      if (doc.exists()) {
        setUserProfile(doc.data() as LeaderboardEntry);
      } else {
        // Create initial profile
        const initialProfile: LeaderboardEntry = {
          name: user.displayName || 'Young Scientist',
          totalScore: 0,
          chaptersCompleted: 0,
          lastActive: serverTimestamp()
        };
        setDoc(profileRef, initialProfile).catch(err => handleFirestoreError(err, OperationType.CREATE, `users/${user.uid}`));
      }
    });
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        entries.push({ ...(doc.data() as LeaderboardEntry), rank: entries.length + 1 });
      });
  ...
  const handleNextQuestion = async () => {
    if (selectedChapter && currentQuestionIndex < selectedChapter.questions.length - 1) {
      playSound(SOUNDS.CLICK);
      ...
      // Calculate final score and sync with Firebase
      if (selectedChapter && user) {
        const correctCount = quizResults.filter(r => r.grade.isCorrect).length;
        const totalCount = selectedChapter.questions.length;
        const finalScore = Math.round((correctCount / totalCount) * 100);

        const progressRef = doc(db, 'users', user.uid, 'progress', selectedChapter.id);
        const profileRef = doc(db, 'users', user.uid);
        ...
        try {
          ...
          await setDoc(progressRef, {
            ...
            attempts: (progress[selectedChapter.id]?.attempts || 0) + 1,
          ...
          await updateDoc(profileRef, {
            totalScore: (userProfile?.totalScore || 0) + scoreDiff,
            chaptersCompleted: (userProfile?.chaptersCompleted || 0) + (isNewChapter ? 1 : 0),
            lastActive: serverTimestamp()
          });
        } catch (err) {
          handleFirestoreError(err, OperationType.WRITE, `users/${user.uid}/progress/${selectedChapter.id}`);
        }
      }
      setScreen('RESULTS');
    }
  ...
  const handleSubmitAnswer = async () => {
    if (!studentAnswer.trim() || !selectedChapter) return;
    
    setIsGrading(true);
    const question = selectedChapter.questions[currentQuestionIndex];
    const grade = await gradeAnswer(question.question, question.answer, studentAnswer, difficulty);
  ...
  const averageScore = progressValues.length > 0 
    ...
    : 0;
  const userProgressMsg = user ? getEncouragement(averageScore, completedChapters) : null;
          ...
          {user && (
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-[#F1EDE7] rounded-full border border-[#DED9D2]">
              <div className="w-6 h-6 bg-[#8BA888] rounded-full flex items-center justify-center text-[10px] text-white">
                {user.photoURL ? <img src={user.photoURL} className="rounded-full" /> : <UserIcon size={12} />}
              </div>
          ...
          {selectedChapter && screen === 'QUIZ' && (
            <div className="flex items-center gap-4">
              <div className="w-24 sm:w-32 h-3 bg-[#E5E0D8] rounded-full relative overflow-hidden">
                <motion.div 
                  initial={{ width: 0 }}
                  animate={{ width: `${((currentQuestionIndex) / selectedChapter.questions.length) * 100}%` }}
          ...
          {screen === 'SELECT_DIFFICULTY' && selectedChapter && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { id: 'EASY', label: 'Easy Peasy', desc: 'More hints, lenient grading', color: '#8BA888', bg: '#F7FAF5', emoji: '🌱' },
                  { id: 'MEDIUM', label: 'Standard', desc: 'Balanced challenge', color: '#D48D6C', bg: '#FFF9F4', emoji: '🌿' },
                  { id: 'HARD', label: 'Expert Explorer', desc: 'Strict terms, precise answers', color: '#D46C6C', bg: '#FFF4F4', emoji: '🌳' }
                ].map((level) => (
                  <motion.button
                    key={level.id}
              ...
              <div className="bg-white p-6 rounded-3xl border-2 border-[#E5E0D8] shadow-sm">
                <div className="flex justify-between items-end mb-3">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[#A39E93] font-bold text-sm"> / {selectedChapter.questions.length}</span>
                  </div>
                </div>
                <div className="h-4 w-full bg-[#F1EDE7] rounded-full overflow-hidden border border-[#E5E0D8] relative">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${((currentQuestionIndex + 1) / selectedChapter.questions.length) * 100}%` }}
                ...
                <div className="natural-card p-8 md:p-10 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-8">
                     ...
                     <div className="flex gap-1.5 overflow-hidden max-w-[150px] sm:max-w-none">
                       {selectedChapter.questions.map((_, i) => (
                         <div 
                           key={i} 
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-[#4A453F] leading-snug mb-8">
                    {currentQuestion.question}
                  </h2>

                  <div className="flex-1 flex flex-col">
                    <label className="text-xs font-bold text-[#A39E93] uppercase mb-2 ml-2 tracking-wider italic">Your Explanation</label>
                    ...
                    {!lastGrade ? (
                        ...
                        <div className="mt-8 flex justify-end gap-4">
                           <button 
                            ...
                            className={`cute-button cute-button-green min-w-[200px] flex items-center justify-center gap-2 ${
                              (!studentAnswer.trim() || isGrading) ? 'opacity-50 grayscale' : ''
                            }`}
                           >
                             {isGrading ? (
                      ...
                      >
                        ...
                        <div className="mt-auto pt-6 flex justify-end">
                           ...
                           >
                             {currentQuestionIndex < selectedChapter.questions.length - 1 ? 'Next Step →' : 'Complete Adventure →'}
                           </button>
                        </div>
                      </motion.div>
                    )}
